...
std::vector<ll> segment_tree(4*5*100005),arr(5*100005);
...
ll query(ll index,ll start,ll end,ll qstart,ll qend,ll val)
{
    if(start>end || start>qend || end<qstart)
        return 0;   
    if(start>=qstart && end<=qend)
    {
        //debug4(start,end,val,segment_tree[index]);
        if(segment_tree[index]%val==0)
            return 0;
        if(start==end)
            return 1;
        ll co = 0;
        if(segment_tree[2*index+1]%val!=0) co++;
        if(segment_tree[2*index+2]%val!=0) co++;
        if(co>1)
            return 2;
    }
    ll mid = start+(end-start)/2;
    return (query(2*index+1,start,mid,qstart,qend,val)+query(2*index+2,mid+1,end,qstart,qend,val));
}
int main()
{
    fastio();
    ...
    {  
        ...
        for(ll i=0;i<q;i++)
        {
            ...
            if(type==1)
            {
                ...
                a--;
                b--;
//                debug1(query(0,0,n-1,a,b,c));
                if(query(0,0,n-1,a,b,c)>1)
                    cout << "NO" << endl;
                else
                    cout << "YES" << endl;
            }
