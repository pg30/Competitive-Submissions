...
//            arr[i][j] = 10;
    ...
    ar<ll,2> dp[n][n];
    fo(i,n) fo(j,n) dp[i][j] = {INF,INF};

    dp[0][0] = {log2(arr[0][0])*1ll,log5(arr[0][0])};
    ...
    fo(i,n)
    {
        fo(j,n)
        {
            ...
            if(i>0)
            {
                dp[i][j][0] = min(dp[i][j][0],dp[i-1][j][0]+log2(arr[i][j])*1ll);
                dp[i][j][1] = min(dp[i][j][1],dp[i-1][j][1]+log5(arr[i][j]));
            }

            if(j>0)
            {
                dp[i][j][0] = min(dp[i][j][0],dp[i][j-1][0]+log2(arr[i][j])*1ll);
                dp[i][j][1] = min(dp[i][j][1],dp[i][j-1][1]+log5(arr[i][j]));
            }
        }
    }
    ...
    cout << ans << endl;
    ll i=n-1,j=n-1;
    bool a = (ans==dp[n-1][n-1][0]);
    bool b = (ans==dp[n-1][n-1][1]);
    while(1)
    {
//        debug2(i,j);
        if(i==0 && j==0) break;
        ...
        if(i-1>=0)
        {
            if(a && dp[i-1][j][0]==dp[i][j][0]-log2(arr[i][j])*1ll)
            {
                p+='D';
                i--;
                continue;
            }
            if(b && dp[i-1][j][1]==dp[i][j][1]-log5(arr[i][j]))
            {
                p+='D';
                i--;
                continue;
        ...
        if(j-1>=0)
        {
            if(a && dp[i][j-1][0]==dp[i][j][0]-log2(arr[i][j]*1ll))
            {
                p+='R';
                j--;
                continue;
            }
            if(b && dp[i][j-1][1]==dp[i][j][1]-log5(arr[i][j]))
            {
                p+='R';
                j--;
                continue;
