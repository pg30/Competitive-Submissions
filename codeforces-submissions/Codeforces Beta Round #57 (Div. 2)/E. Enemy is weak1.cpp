...
std::vector<ll> segment_tree(4*1000005),arr(1000005);
void update(ll index,ll start,ll end,ll val)
{
    if(start>end)
        return;
    if(start==end)
    ...
    if(val<=mid)
        update(2*index+1,start,mid,val);
    else
        update(2*index+2,mid+1,end,val);
...
int main()
{
    ...
    { 
        ...
        for(ll i=0;i<n;i++)
        {
            arr[i] = lower_bound(all(temp),arr[i])-temp.begin();
        }
        ll left[n];
        ll ans = 0;
        for(ll i=0;i<n;i++)
        {
            left[i] = query(0,0,n-1,arr[i]+1,n-1);
            update(0,0,n-1,arr[i]);
        }
        ll right[n];
        fill(all(segment_tree),0);
        for(ll i=n-1;i>=0;i--)
        {
            right[i] = query(0,0,n-1,0,arr[i]-1);
            update(0,0,n-1,arr[i]);
        }
        // printArray(left,n);
        // printArray(right,n);
