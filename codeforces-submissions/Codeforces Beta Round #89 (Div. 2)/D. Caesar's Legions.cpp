...
void solve()
{
    ...
    ll dp[n1 + 1][n2 + 1][2]; //pos footmen horsemen
    ...
    for (ll j = 0; j <= n1; j++)
    {
        for (ll k = 0; k <= n2 ; k++)
        {
            for (ll a = 1; j - a >= 0 && a <= k1; a++)
                dp[j][k][0] = add(dp[j][k][0], dp[j - a][k][1], mod);
            for (ll a = 1; k - a >= 0 && a <= k2; a++)
                dp[j][k][1] = add(dp[j][k][1], dp[j][k - a][0], mod);
        }
    }
    cout << add(dp[n1][n2][0], dp[n1][n2][1], mod) << endl;
}
int main()
{
    fastio();
