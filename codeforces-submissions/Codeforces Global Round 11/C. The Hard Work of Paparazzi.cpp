...
void solve()
{
    ...
    for(ll i=1;i<n;i++)
    {
        if(arr[i][1]+arr[i][2]-2 <= arr[i][0])
            dp[i] = 1;
        for(ll j=max(0ll,i-2*r);j<i;j++)
        {
            if(dp[j]>0 && abs(arr[i][1]-arr[j][1])+abs(arr[i][2]-arr[j][2])<=abs(arr[i][0]-arr[j][0]))
                dp[i] = max(dp[i],dp[j]+1);
        }
        if(i-2*r>0)
            dp[i] = max(dp[i],pref[i-2*r-1]+1);
        pref[i] = max(pref[i-1],dp[i]);
    }

//    printArray(dp,n);

    cout << *max_element(alla(dp,n)) << endl;
}
int main()
{
    fastio();
