...
void solve2d(std::vector<data> arr)
{
    if (arr.size() == 1)
    {
        temp.pb(arr[0]);
    ...
    map<ll, std::vector<data> > naya;
    for (auto i : arr)
        naya[i.y].pb(i);

    std::vector<data> dekh;

    for (auto i : naya)
    {
        sort(all(i.se), [](data a, data b) {
            return a.x < b.x;
        });
...
void solve()
{
    ...
    map<ll, std::vector<data> > pos;
    for (ll i = 0; i < n; i++)
        pos[arr[i].z].pb(arr[i]);
