...
int main()
{
    ...
    while(t--)
    {
        ...
        for(ll i=0;i<len;i++)
            ans+=str[i];
        string back = "";
        for(ll i=n-1;i>=n-len;i--)
            back+=str[i];
        reverse(all(back));
        str = str.substr(len,n-2*len);
        string revstring = str;
        reverse(all(revstring));
        string a = str+'+'+revstring,b = revstring+'+'+str;
        std::vector<ll> tempa = prefix_function(a),tempb = prefix_function(b);
        ll ansa = tempa[a.length()-1],ansb = tempb[b.length()-1];
        if(ansa>ansb)
        {
            for(ll i=0;i<ansa;i++)
                ans+=a[i];
