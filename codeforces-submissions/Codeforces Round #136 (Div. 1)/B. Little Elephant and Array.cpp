...
void solve()
{
    ...
    ll k = ceil(sqrt(n) * 1.0);
    sort(all(query), [&](data a, data b) {
        ll x1 = a.l, x2 = b.l;
        if (x1 / k != x2 / k) return x1 / k < x2 / k;
        return a.r < b.r;
    });
    ...
    for (ll i = 0; i < m; i++)
    {
        ll x = query[i].l, y = query[i].r;
        while (l < x)
        {
            if (arr[l] <= 1e5)
            {
                cnt[arr[l]]--;
                if (cnt[arr[l]] == arr[l] - 1 && st.count(arr[l]))
                    st.erase(arr[l]);
                if (cnt[arr[l]] == arr[l])
                    st.insert(arr[l]);
            }
        ...
        while (r < y)
        {
            ...
            if (arr[r] <= 1e5)
            {
                ...
                if (cnt[arr[r]] == arr[r])
                    st.insert(arr[r]);
                if (cnt[arr[r]] == arr[r] + 1 && st.count(arr[r]))
                    st.erase(arr[r]);
            }
        }
        while (r > y)
