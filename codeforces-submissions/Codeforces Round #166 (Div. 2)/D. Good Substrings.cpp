...
void solve()
{
    ...
    const int p = 13331, m = 1e9 + 9;
    const int p1 = 7919 , m1 = 1e9 + 9;
    ll ppow[n], ppow1[n];
    ppow[0] = 1;
    ppow1[0] = 1;
    for (ll i = 1; i < n; i++)
    {
        ppow[i] = multiply(ppow[i - 1], p, m);
        ppow1[i] = multiply(ppow1[i - 1], p1, m1);
    }
    ...
    for (ll i = 1; i < n; i++)
    {
        hash[i] = add(hash[i - 1], multiply((str[i] - 'a' + 1), ppow[i], m), m);
        hash1[i] = add(hash1[i - 1], multiply((str[i] - 'a' + 1), ppow1[i], m1), m1);
    }
    ...
    for (ll i = 0; i < n; i++)
    {
        for (ll j = i; j < n; j++)
        {
            ...
            for (ll a = 0; a < 26; a++)
            {
                if (good[a] == '0')
                {
                    bad_cnt += cnt[j][a] - (i > 0 ? cnt[i - 1][a] : 0);
                }
            }
            is_good = (bad_cnt <= k);
            if (is_good)
            {
                ll currhash = substract(hash[j] , (i > 0 ? hash[i - 1] : 0), m);
                currhash = multiply(currhash, ppow[n - i - 1], m);
                ll currhash1 = substract(hash1[j] , (i > 0 ? hash1[i - 1] : 0), m);
                currhash1 = multiply(currhash1, ppow1[n - i - 1], m1);
                st.insert({currhash, currhash1});
            }
        }
    }
    cout << st.size() << endl;
