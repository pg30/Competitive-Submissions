...
string str;
ll m;
map<pair<ll, pi>, ll> dp;
bool util(ll pos, ll prev, ll balance) // pos,prev,balance
{
    if (pos == m)
    {
        return true;
    ...
    if (balance > 10 || balance < -10)
        return false;
    if (dp.find({pos, {prev, balance}}) != dp.end())
        return dp[ {pos, {prev, balance}}];
    bool curr = false;
    for (ll i = 0; i < 10; i++)
    {
        if (str[i] == '1' && i + 1 != prev)
        {
            ...
            if ((pos % 2 == 0 && b > 0) || (pos % 2 != 0 && b < 0))
            {
                curr = curr || util(pos + 1, i + 1, b);
                if (curr)
                {
//                    debug1(pos);
                    ans.pb(i + 1);
                    dp[ {pos, {prev, balance}}] = 1;
                    return true;
                }
            }
        }
    }
    dp[ {pos, {prev, balance}}] = 0;
    return false;
}
void solve()
{
