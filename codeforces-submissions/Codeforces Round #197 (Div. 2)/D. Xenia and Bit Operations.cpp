...
std::vector<ll> segment_tree(4*((1<<17)+1)),arr((1<<17)+1);
void build(ll index,ll start,ll end,ll level)
{
    if(start>end)
        return;
    if(start==end)
    ...
    build(2*index+1,start,mid,level-1);
    build(2*index+2,mid+1,end,level-1);
    if(!(level&1))
    {
//        debug2(level,"or");
...
void update(ll index,ll start,ll end,ll pos,ll val,ll level)
{
    if(start>end)
        return;
    ...
    if(pos>=start && pos<=mid)
        update(2*index+1,start,mid,pos,val,level-1);
    else
        update(2*index+2,mid+1,end,pos,val,level-1);
    if(!(level&1))
        segment_tree[index] = segment_tree[2*index+1]|segment_tree[2*index+2];
    else
        segment_tree[index] = segment_tree[2*index+1]^segment_tree[2*index+2];
...
int main()
{
    ...
    while(t--)
    {
        ...
        for(ll i=0;i<sz;i++)
            cin >> arr[i];
        build(0,0,sz-1,n+1);
//        debug1(segment_tree[0]);
        for(ll i=0;i<m;i++)
        {
            ll p,b;
            cin >> p >> b;
            p--;
            update(0,0,sz-1,p,b,n+1);
            cout << segment_tree[0] << endl;
        }
