...
//---------------TEMPLATE ABOVE--------------//
bool isSafe(ll n, ll m, pi a)
{
    if (a.fi >= 0 && a.fi < n && a.se >= 0 && a.se < m)
        return true;
    return false;
}
ll dx[] = {0, -1, 0, 1};
...
int main()
{
    ...
    while (t--)
    {
        ...
        while (cnt < ecnt - k )
        {
            ...
            {
                cnt++;
                arr[point.fi][point.se] = '1';
                visited[point.fi][point.se] = 1;
                fo(i, 0, 4)
                {
                    pi temp = point;
                    temp.fi += dx[i];
                    temp.se += dy[i];
                    if (isSafe(n, m, temp) && arr[temp.fi][temp.se] == '.' && !visited[temp.fi][temp.se])
                    {
                        pq.push(temp);
                    }
                }
