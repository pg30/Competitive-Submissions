...
public:
    ...
    void combine(ll ind,ll s,ll e)
    {
        if(s==e)
        {
            tree[ind] = {s,arr[s]==0?INT_MAX:arr[s]};
            return;
        }
        if(tree[2*ind+1].se < tree[2*ind+2].se)
    ...
    pi query(ll ind,ll s,ll e,ll l,ll r)
    {
        if(s>e || e<l || s>r)
            return {-1,INT_MAX};
        if(s>=l && e<=r)
            return tree[ind];
        ll m = s+(e-s)/2;
        pi a = query(2*ind+1,s,m,l,r);
        pi b = query(2*ind+2,m+1,e,l,r);
...
ll util(vi &arr,ll s,ll e,SegmentTree &seg,ll h)
{
    if(s>e)
        return 0;
    ll n = arr.size();
    pi curr = seg.query(0,0,n-1,s,e);

    if(curr.se==INT_MAX )
        return util(arr,s,curr.fi-1,seg,0)+util(arr,curr.fi+1,e,seg,0);

    if(curr.se-h<=0)
        return util(arr,s,curr.fi-1,seg,h)+util(arr,curr.fi+1,e,seg,h);
    
    //ll ans = 1+util(arr,s,curr.fi-1,seg,h)+util(arr,curr.fi+1,e,seg,h);
    ll ans = min(e-s+1,curr.se-h+util(arr,s,curr.fi-1,seg,curr.se)+util(arr,curr.fi+1,e,seg,curr.se));
    return ans;
}
void solve()
{
    ...
    SegmentTree seg(arr);
    seg.build(0,0,n-1);
    cout << util(arr,0,n-1,seg,0) << endl;
}
int main()
{
    fastio();
