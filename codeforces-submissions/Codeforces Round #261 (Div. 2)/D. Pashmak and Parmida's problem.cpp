...
std::vector<ll> BIT(4*1000005),arr(1000005);
void addd(ll index,ll val,ll n)
{
    index++;
    while(index<=n)
    {
...
int main()
{
    ...
    {  
        ...
        std::vector<ll> left(n),right(n),cnt(n);
        for(ll i=0;i<n;i++)
        {
            cnt[arr[i]]++;
            left[i] = cnt[arr[i]];
