...
ll solve(ll pos,string str,ll final,ll curr)
{
    if(pos==str.length())
        return curr==final;
    if(str[pos]=='+')
    ...
    {
        return (solve(pos+1,str,final,curr+1)+solve(pos+1,str,final,curr-1));
    }
...
//    cin >> t;
    ...
    {   
        ...
        for(auto i:str2)
            question+=(i=='?');
        cout << fixed << setprecision(9) << (solve(0,str2,final,0)*1.0/(1<<question)) << endl;
