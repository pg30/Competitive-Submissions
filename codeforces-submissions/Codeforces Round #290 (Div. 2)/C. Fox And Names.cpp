...
set<ll> vis;
vi graph[26];
int check[26]; // 0 white 1 gray 2 black
bool check_cycle(ll i)
{
...
void solve()
{
    ...
    {
        bool ok = false;
        for (ll j = 0 ; j < min(arr[i].length(), arr[i + 1].length()); j++)
        {
            if (arr[i][j] != arr[i + 1][j])
            {
                if (find(all(graph[arr[i + 1][j] - 'a']), arr[i][j] - 'a') != graph[arr[i + 1][j] - 'a'].end())
                {
                    cout << "Impossible" << endl;
                    return;
                }
                ok = true;
                if (find(all(graph[arr[i][j] - 'a']), arr[i + 1][j] - 'a') == graph[arr[i][j] - 'a'].end())
                {
                    graph[arr[i][j] - 'a'].pb(arr[i + 1][j] - 'a');
                }
                break;
            }
        }
