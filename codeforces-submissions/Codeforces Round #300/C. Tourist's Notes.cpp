...
void solve()
{
    ll n, m;
    cin >> n >> m;
    vii(pi) arr(m); // day,height
    for (ll i = 0; i < m; i++)
        cin >> arr[i].fi >> arr[i].se;

    for (ll i = 0; i < m - 1; i++)
    {
        if (abs(arr[i + 1].se - arr[i].se) > arr[i + 1].fi - arr[i].fi)
        {
    ...
    ll ans = max(arr[0].fi + arr[0].se - 1, arr[m - 1].se + n - arr[m - 1].fi);
    for (ll i = 0; i < m - 1; i++)
    {
        ll mini = min(arr[i].se, arr[i + 1].se), maxi = max(arr[i].se, arr[i + 1].se);
        ll temp = mini + arr[i + 1].fi - arr[i].fi;
        ll curr = temp - ceil((temp - maxi) * 1.0 / 2);
//        debug3(temp, curr, ans);
        ans = max(ans, curr);
    }
    cout << ans << endl;
}
