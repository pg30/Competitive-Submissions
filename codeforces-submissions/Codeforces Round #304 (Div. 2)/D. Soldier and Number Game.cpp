...
void solve()
{
    ...
    // for (ll i = 2; i < 10; i++)
    //     debug2(i, cnt_of_prime[i]);

    for (ll i = 2; i < maxn; i++)
        cnt_of_prime[i] += cnt_of_prime[i - 1];
    ...
    for (ll i = 0; i < n; i++)
    {
        ...
        ll s = b + 1, e = a;
        // debug2(s - 1, e);
        // debug2(cnt_of_prime[s - 1], cnt_of_prime[e]);
        cout << cnt_of_prime[e] - cnt_of_prime[s - 1] << endl;
    }
}
int main()
{
