...
bool util(string a, string b)
{
    ...
    if (a.size() % 2 == 1)
        return false;
    return dp[ {a, b}] = ((util(a.substr(0, a.size() / 2), b.substr(0, b.size() / 2)) && util(a.substr(a.size() / 2, a.size() / 2), b.substr(b.size() / 2, b.size() / 2)))
                          || (util(a.substr(0, a.size() / 2), b.substr(b.size() / 2, b.size() / 2)) && util(a.substr(a.size() / 2, a.size() / 2), b.substr(0, b.size() / 2))) );
}
void solve()
{
    string a, b;
