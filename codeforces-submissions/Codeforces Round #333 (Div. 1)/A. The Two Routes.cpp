...
//---------------TEMPLATE ABOVE--------------//
ll shortest_distance(vi graph[], ll n)
{
    ll s = 0;
    queue<ll> q;
    q.push(s);
...
int main()
{
    ...
    while (t--)
    {
        ll n, m;
        cin >> n >> m;
        vi railway[n], roadway[n];
        bool dir_railway = false, dir_road = false;
        fo(i, 0, m)
        {
            ll a, b;
            cin >> a >> b;
        ...
        fo(i, 0, n)
        {
            fo(j, 0, n)
            {
                if (find(all(railway[i]), j) == railway[i].end())
                    roadway[i].pb(j);
            }
        }
        ll ans = 1;
        if (dir_road)
        {
            ll temp = shortest_distance(railway, n);
            if (temp == INT_MAX)
                ans = -1;
            else
                ans = max(ans, temp);
        ...
        {
            ll temp = shortest_distance(roadway, n);
            if (temp == INT_MAX)
                ans = -1;
            else
                ans = max(ans, temp);
