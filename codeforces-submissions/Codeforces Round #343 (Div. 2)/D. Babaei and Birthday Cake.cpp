...
ll query(ll index,ll start,ll end,ll qstart,ll qend)
{
    ...
    ll mid = start+(end-start)/2;
    return max(query(2*index+1,start,mid,qstart,qend),query(2*index+2,mid+1,end,qstart,qend));
}
void update(ll index,ll start,ll end,ll pos,ll val)
{
    ...
    {
        seg_tree[index]=max(val,seg_tree[index]);
        return;
    }
    ll mid = start+(end-start)/2;
    if(pos<=mid)
...
int main()
{
    ...
    {  
        ll n;
        cin >> n;
        std::vector<pair<ll,ll> > arr(n),temp(n);
        for(ll i=0;i<n;i++)
        {
            cin >> arr[i].fi >> arr[i].se;
            temp[i].fi = arr[i].fi*arr[i].fi*arr[i].se;
            //update(0,0,n-1,i,temp[i].fi);
            temp[i].se = i;
        }
        sort(all(temp),cmp);
        std::vector<char> v;
        ...
        for(ll i=0;i<n;i++)
        {
            dp[i]=temp[i].fi+query(0,0,n-1,0,temp[i].se-1);
            //debug1(temp[i].se);
            update(0,0,n-1,temp[i].se,dp[i]);
        }
//        printArray(dp,n);
        cout << fixed << setprecision(9) << *max_element(alla(dp,n))*1.0*PI << endl; 
