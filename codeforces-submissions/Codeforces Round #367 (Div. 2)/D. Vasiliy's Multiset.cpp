...
public:
    ...
    void del(ll val)
    {
        root = delUtil(root, val, 32);
    }

    TrieNode *delUtil(TrieNode *temp, ll val, ll pos)
    {
        if (pos == -1)
        {
            temp->cnt--;
        ...
        bool bit = (val & (1ll << pos)) == 0 ? 0 : 1;
        temp->arr[bit] = delUtil(temp->arr[bit], val, pos - 1);
        if (temp->arr[0] == NULL && temp->arr[1] == NULL && !temp->isLeaf)
            return NULL;
        return temp;
    }
