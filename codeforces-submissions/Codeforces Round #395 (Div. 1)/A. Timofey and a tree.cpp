...
vi graph[size];
vi arr(size);
bool dfs(ll pos, ll n, ll res, ll par, ll color)
{
    bool ok = true;
    for (auto i : graph[pos])
    {
        if (i != par)
        {
            ok = (ok && (arr[i] == color || color == -1) && dfs(i, n, res, pos, arr[i]));
        }
    }
    return ok;
}
int main()
{
    ...
    while (t--)
    {
        ...
        left = dfs(diff.fi, n, diff.se, -1, -1);
        right = dfs(diff.se, n, diff.fi, -1, -1);
