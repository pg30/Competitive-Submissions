...
void solve()
{
    ...
    vii(pi) pref(n), suff(n);

    pref[0] = arr[0];
    for (ll i = 1; i < n; i++)
    {
        if (arr[i].fi > pref[i - 1].se)
            pref[i] = { -1, -1};
        else
            pref[i] = {arr[i].fi, min(pref[i - 1].se, arr[i].se)};
    }

    suff[n - 1] = arr[n - 1];
    for (ll i = n - 2; i >= 0; i--)
    {
        if (arr[i].se < suff[i + 1].fi)
            suff[i] = { -1, -1};
        else
            suff[i] = {suff[i + 1].fi, min(suff[i + 1].se, arr[i].se)};
    }

    // for (ll i = 0; i < n; i++)
    //     debug2(pref[i].fi, pref[i].se);

    ll ans = max(pref[n - 2].se - pref[n - 2].fi, suff[1].se - suff[1].fi, 0ll);
    for (ll i = 1; i < n - 1; i++)
    {
        pi a = {max(pref[i - 1].fi, suff[i + 1].fi), min(pref[i - 1].se, suff[i + 1].se)};
        ans = max(ans, a.se - a.fi, 0ll);
    }
    cout << ans << endl;
}
int main()
