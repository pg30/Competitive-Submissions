...
void solve()
{
    ll n, h;
    cin >> n >> h;
    std::vector<std::vector<ll> > arr(n, std::vector<ll>(2));
    for (ll i = 0; i < n; i++)
        cin >> arr[i][0] >> arr[i][1];

    if (n == 1)
    {
        cout << arr[0][1] - arr[0][0] + h << endl;
        return;
    }

    ll pref[n - 1];
    pref[0] = arr[1][0] - arr[0][1];
    for (ll i = 1; i < n - 1; i++)
        pref[i] = pref[i - 1] + arr[i + 1][0] - arr[i][1];

    ll ans = 0;
    for (ll i = 0; i < n; i++)
    {
        ll s = i + 1, e = n - 1, curr = i;
        while (s <= e)
        {
            ll m = s + (e - s) / 2;
            ll now = pref[m - 1] - (i > 0 ? pref[i - 1] : 0);
//            debug3(i, m, now);
            if (now < h)
            {
                curr = m;
        ...
        ll temp = arr[curr][1] - arr[i][0];
//        debug3(i, curr, temp);
        curr = temp + h - (curr > 0 ? (pref[curr - 1] - (i > 0 ? pref[i - 1] : 0)) : 0);
        ans = max(ans, curr);
    }
    cout << ans << endl;
}
