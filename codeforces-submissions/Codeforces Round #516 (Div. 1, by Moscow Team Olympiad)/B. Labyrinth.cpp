...
void solve()
{
    ...
    vector<vector<pi>> ans(n, vector<pi>(m, {INF, INF}));
    ans[r][c] = {0, 0};
    deque<pi> q;
    q.push_front({r, c});
    while (!q.empty())
    {
        ...
        for (ll i = 0; i < 4; i++)
        {
            ...
            if (isSafe(v.fi, v.se, n, m) && arr[v.fi][v.se] != '*')
            {
                ll w = v.se - u.se;
                if (w == -1)
                {
                    if (ans[u.fi][u.se].fi + 1 < ans[v.fi][v.se].fi)
                    {
                        ans[v.fi][v.se] = {ans[u.fi][u.se].fi + 1, ans[u.fi][u.se].se};
                        q.push_back(v);
                    }
                }
                else if (w == 1)
                {
                    if (ans[u.fi][u.se].se + w < ans[v.fi][v.se].se)
                    {
                        ans[v.fi][v.se] = {ans[u.fi][u.se].fi, ans[u.fi][u.se].se + 1};
                        q.push_back(v);
                    }
                }
                else
                {
                    if (ans[u.fi][u.se].se < ans[v.fi][v.se].se)
                    {
                        ans[v.fi][v.se] = ans[u.fi][u.se];
                        q.push_front(v);
                    }
                }
            }
