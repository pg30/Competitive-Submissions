...
int main()
{
    ...
    ll q;
    cin >> q;
    std::vector<pair<ll,pair<ll,ll> > > qu(q);
    for(ll i=0;i<q;i++)
    { 
        cin >> qu[i].fi;
        if(qu[i].fi==1)
            cin >> qu[i].se.fi >> qu[i].se.se;
        else
            cin >> qu[i].se.fi;
    }
    bool is[n+1];
    ...
    for(ll i=q-1;i>=0;i--)
    {
        if(qu[i].fi==1 && !is[qu[i].se.fi])
        {
            arr[qu[i].se.fi] = max(maxnow,qu[i].se.se);
            is[qu[i].se.fi] = true;
        }
        else if(qu[i].fi==2)
        {
            maxnow = max(maxnow,qu[i].se.fi);
        }
    }
    for(ll i=1;i<=n;i++)
    {
        if(!is[i])
            arr[i] = max(arr[i],maxnow);
        cout <<  arr[i] << " ";
    }
    cout << endl;
}
