...
//    cin >> t;
    while(t--)
    {
        ...
        cin >> str;
        ll ans = 0;
        std::vector<pair<ll,ll> > arr(n);
        for(ll i=0;i<n;i++) cin >> arr[i].fi >> arr[i].se;
        ll v[n][1001];
        for(ll i=0;i<n;i++)
        {
            ll a = arr[i].fi,b = arr[i].se;
            for(ll j=0;j<b;j++){ str[i]=='0'?v[i][j]=0:v[i][j]=1; }
            str[i]=='0'?str[i]='1':str[i]='0';
            for(ll j=b;j<1000;j+=a)
            {
                for(ll k=j;k<j+a && k<1000;k++)
                {
                    str[i]=='0'?v[i][k]=0:v[i][k]=1;
                }
                str[i]=='0'?str[i]='1':str[i]='0';
            }
        }
