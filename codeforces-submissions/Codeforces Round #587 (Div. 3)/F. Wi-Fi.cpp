...
void solve()
{
    ...
    for(ll i=1;i<n;i++)
    {
        ...
        dp[i][0] = i+1+min(dp[i-1][1],dp[i-1][0]);
        if(curr[0]!=-1)
            dp[i][0] = min(dp[i][0],curr[0]);
        ...
        if(str[i]=='1')
        {
            ...
            dp[i][1] = (i-k-1) < 0 ? i+1 : min(dp[i-k-1][0],dp[i-k-1][1])+i+1;
            dp[i][1] = min(dp[i][1],i+1+curr[0]);
            st.insert({dp[i][1],i});
        }
        else
        {
        ...
        temp.insert({min(dp[i][0],dp[i][1]),i});
    }
//    for(ll i=0;i<n;i++)
