...
void solve()
{
    ...
    for (ll i = 0; i < n; i++)
    {
        ...
        {
            ll mini = min(i, map[node].fi);
            ll maxi = max(i, map[node].se);
            map[node] = {mini, maxi};
        }
    }
    ...
    for (auto i : map)
    {
//        debug2(i.se.fi, i.se.se);
        arr.pb({i.se.fi, i.se.se});
    }
    ...
    {
        j = i + 1;
        ll mi = arr[i].fi, ma = arr[i].se;
        while (j < arr.size() && arr[j].fi <= ma && arr[j].fi >= mi)
        {
            mi = min(mi, arr[j].fi);
            ma = max(ma, arr[j].se);
            ans++;
