...
ll solve(ll a,ll b,ll c,ll pos)
{
	if(pos==3)
		return (abs(a-b)+abs(a-c)+abs(b-c));
	if(pos==0)
	{
		ll ans = min(solve(a,b,c,pos+1),solve(a+1,b,c,pos+1),solve(a-1,b,c,pos+1));
		return ans;
	}
	else if(pos==1)
	{
		ll ans = min(solve(a,b,c,pos+1),solve(a,b+1,c,pos+1),solve(a,b-1,c,pos+1));
		return ans;
	}
	else if(pos==2)
	{
		ll ans = min(solve(a,b,c,pos+1),solve(a,b,c+1,pos+1),solve(a,b,c-1,pos+1));		
		return ans;
	}
}
int main()
{
    ...
    while(t--)
    {
    	...
    	sort(arr,arr+3);
    	cout << solve(arr[0],arr[1],arr[2],0) << endl;
