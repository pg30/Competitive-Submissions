...
void solve()
{
    ...
    ll dp[n][n][2]; // pos even curr : even/odd
    for (ll i = 0; i < n; i++)
    {
        for (ll j = 0; j < n; j++)
    ...
    if (arr[0] != 0)
        dp[0][(arr[0] % 2) == 1 ? 0 : 1][arr[0] % 2] = 0;
    else
        dp[0][0][1] = dp[0][1][0] = 0;

    for (ll i = 1; i < n; i++)
    {
        if ((arr[i] == 0 || arr[i] % 2 == 1) && (i + 1) <= odd)
            dp[i][0][1] = dp[i - 1][0][1];
        for (ll j = 1; j <= i + 1; j++)
        {
            if ((i + 1 - j) > odd)
            ...
            if (arr[i] != 0)
            {
                dp[i][j][arr[i] % 2] = min(dp[i - 1][j - (arr[i] % 2 == 0)][0] + (0 != (arr[i] % 2)), dp[i - 1][j - (arr[i] % 2 == 0)][1] + (1 != (arr[i] % 2)));
                continue;
            }
            for (ll a = 0; a < 2; a++)
            {
                for (ll b = 0; b < 2; b++)
                {
                    dp[i][j][a] = min(dp[i][j][a], dp[i - 1][j - (a == 0)][b] + (a != b));
                }
            }
        }
    }
    ...
    //     {
    //         debug4(i, j, 0, dp[i][j][0]);
    //         debug4(i, j, 1, dp[i][j][1]);
    //     }
    // }
    cout << min(dp[n - 1][even][0], dp[n - 1][even][1]) << endl;
}
int main()
{
    fastio();
