...
ll n;
std::vector<ll> arr;
ll solve(std::vector<ll> &arr,ll i)
{
    if(arr.size()==0 || i<0) return 0;
    std::vector<ll> co,cz;
    for(auto &j:arr)
    {
        //debug3(i,j,(j & (1<<i)));
...
//    debug3(co.size(),cz.size(),i);
    if(co.size()==0) return solve(cz,i-1);
    if(cz.size()==0) return solve(co,i-1);
    return min(solve(co,i-1),solve(cz,i-1))+(1<<i);
}
int main()
{
    fastio();
