...
ll dp[2001][2001];
ll solve(ll pos,ll currtime,std::vector<ll> &arr,ll h,ll l,ll r)
{
    if(pos==arr.size())
        return 0;
    if(dp[pos][currtime]!=-1)
        return dp[pos][currtime];
    ll ans = 0;
    ll temp = (currtime+arr[pos])%h;
    ans = (temp>=l && temp<=r)+solve(pos+1,temp,arr,h,l,r);
    temp = (currtime+arr[pos]-1)%h;
    ans = max(ans,(temp>=l && temp<=r)+solve(pos+1,temp,arr,h,l,r));
    return dp[pos][currtime]=ans;
}
int main()
{
    fastio();
