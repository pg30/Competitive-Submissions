...
ll in[MAX],out[MAX];
ll tim=0;
std::vector<ll> adjList[MAX];
void dfs(ll pos,ll parent)
{
    in[pos] = tim++;
    for(auto child:adjList[pos])
    {
        if(child!=parent)
        {
...
int main()
{
    ...
    {  
        ...
        while(m--)
        {
            ...
            sort(all(arr),comp);
            bool ok = true;
            //debug3(arr[k-1],in[arr[k-1]],out[arr[k-1]]);
            for(ll i=0;i<k-1;i++)
            {
                //debug3(arr[i],in[arr[i]],out[arr[i]]);
                if(in[arr[k-1]]>=in[arr[i]] && out[arr[k-1]]<=out[arr[i]])
                {
                    continue;
                }
                ok = false;
                for(auto child:adjList[arr[i]])
                {
                    if((in[arr[k-1]]>=in[child] && out[arr[k-1]]<=out[child]))
                    {
                        ok = true;
                        break;
                    }
