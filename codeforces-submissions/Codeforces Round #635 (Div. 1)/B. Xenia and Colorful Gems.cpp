...
//---------------TEMPLATE ABOVE--------------//
ll util(const vi &a, const vi &b, const vi &c)
{
    ll ans = INF;
    for (ll i = 0; i < b.size(); i++)
    {
        ll i1, i2;
        auto ind1 = lower_bound(all(c), b[i]);
        if (ind1 == c.end())
            i1 = c.size() - 1;
        else
            i1 = ind1 - c.begin();

        ind1 = upper_bound(all(a), b[i]);
        if (ind1 == a.begin())
            i2 = 0;
        else
        {
        ...
        ans = min(ans, (c[i1] - b[i]) * (c[i1] - b[i]) + (a[i2] - b[i]) * (a[i2] - b[i]) + (c[i1] - a[i2]) * (c[i1] - a[i2]));
    }
    return ans;
}
void solve()
{
    ...
    ll ans = INF;
    ans = min(min(util(ar, ag, ab), util(ar, ab, ag), util(ag, ar, ab)), min(util(ag, ab, ar), util(ab, ar, ag), util(ab, ag, ar)));
    cout << ans << endl;
}
int main()
{
