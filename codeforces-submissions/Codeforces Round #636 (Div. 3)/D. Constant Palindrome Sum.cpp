...
void solve()
{
    ...
    vi temp;
    for (ll i = 0; i < n / 2; i++)
        temp.pb(arr[i] + arr[n - i - 1]);
    sort(all(temp));

    ll  cnt[2 * k + 5] = {0}, tcnt[2 * k + 5] = {0};
    for (ll i = 0; i < n / 2; i++)
    {
        cnt[1]++;
        cnt[min(arr[i], arr[n - i - 1]) + 1]--;
        tcnt[max(arr[i], arr[n - i - 1]) + k + 1]++;
    }
    for (ll i = 2; i < 2 * k + 5; i++)
    {
        cnt[i] += cnt[i - 1];
        tcnt[i] += tcnt[i - 1];
    }
//    printArray(cnt, k + 1);
    ...
    for (ll i = 2; i < 2 * k + 5; i++)
    {
        ...
        ll b = cnt[i] + tcnt[i];
        ll curr = c - map[i] + b;
//        debug5(i, curr, map[i], cnt[i], tcnt[i]);
        ans = min(ans, curr);
    }
    cout << ans << endl;
}
