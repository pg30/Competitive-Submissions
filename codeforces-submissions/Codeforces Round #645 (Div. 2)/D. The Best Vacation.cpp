...
void solve()
{
    ...
    ll pref[2 * n] = {0}, value[2 * n] = {0};
    pref[0] = arr[0];
    value[0] = arr[0] * (arr[0] + 1) / 2;
    for (ll i = 1; i < 2 * n; i++)
    {
        pref[i] = pref[i - 1] + arr[i];
        value[i] = value[i - 1] + arr[i] * (arr[i] + 1) / 2;
    }
    ...
    {
        ll tillnow = pref[i];
        if (tillnow < x) continue;

        ll s = 1, e = i + 1, ind = i;
        while (s <= e)
        {
            ll m = s + (e - s) / 2;
            ll sum = pref[i] - (i - m >= 0 ? pref[i - m] : 0);
//            debug3(i, m, sum);
            if (sum >= x)
            {
                ind = m;
        ...
        ind = i - ind + 2;
//        debug2(i, ind);
        ll cnt = pref[i] - (ind > 0 ? pref[ind - 1] : 0);
//        debug1(cnt);
        ll cntsum = value[i] - (ind > 0 ? value[ind - 1] : 0);
        ll temp = x - cnt;
        ll t1 = 0;
        if (ind > 0)
        {
            t1 = arr[ind - 1] * (arr[ind - 1] + 1) / 2;
            t1 -= (arr[ind - 1] - temp) * (arr[ind - 1] - temp + 1) / 2;
        }
        ans = max(ans, cntsum + t1);
    }
    cout << ans << endl;
}
int main()
