...
vi graph[maxn];
ll ans = 0;
ar<ll,2> dfs(ll node,ll par)
{
    if(par!=-1)
        arr[node][0] = min(arr[node][0],arr[par][0]);
    ar<ll,2> curr = {0,0};
    if(arr[node][1]!=arr[node][2])
        curr[arr[node][1]]++;
    for(auto i : graph[node])
    {
        if(i!=par)
        {
            ar<ll,2> now = dfs(i,node);
            curr[0]+=now[0];
            curr[1]+=now[1];
        }
    }
    //debug3(node+1,curr[0],curr[1]);
    ll mini = min(curr[0],curr[1]);
    curr[0]-=mini;
    curr[1]-=mini;
    ans += arr[node][0]*mini*2;
    return curr;
}
void solve()
{
    ...
    ar<ll,2> curr = dfs(0,-1);
    if(curr[0]!=curr[1] || curr[0]!=0)
        cout << -1 << endl;
    else
        cout << ans << endl;
}
