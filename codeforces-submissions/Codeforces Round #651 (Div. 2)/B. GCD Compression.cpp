...
//            debug2(arr[j], arr[j] % i);
        ...
        for (ll j = 1; j < i; j++)
        {
            if (j == i - j)
                ...
                cnt += min(map[j].size(), map[i - j].size());
        }
        cnt += (map[0].size() / 2);
//        debug2(i, cnt);
        if (cnt >= n - 1)
        {
            while (!map[0].empty() && map[0].size() >= 2)
            {
                ll a = map[0].back();
                map[0].pop_back();
                ll b = map[0].back();
...
//                debug2(a, b);
            ...
            for (ll j = 1; j < i; j++)
            {
                while (!map[j].empty() && !map[i - j].empty())
                {
                    ll a = map[j].back();
                    map[j].pop_back();
                    if (map[i - j].empty()) break;
                    ll b = map[i - j].back();
                    map[i - j].pop_back();
                    ans.pb({a, b});
                }
            ...
            for (ll j = 0; j < n - 1; j++)
            {
                cout << ans[j].fi + 1 << " " << ans[j].se + 1 << endl;
            }
            return;
        }
    }
