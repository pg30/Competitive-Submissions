...
void solve()
{
    ...
    vi prefboth, prefa, prefb;

    if (!both.empty())
    {
        prefboth.pb(both[0]);
        for (ll i = 1; i < both.size(); i++)
            prefboth.pb(both[i] + prefboth[i - 1]);
    }
    ...
    if (!b.empty())
    {
        ...
        for (ll i = 1; i < b.size(); i++)
            prefb.pb(b[i] + prefb[i - 1]);
    }

    if (both.empty())
    {
        if (prefa.size() < k || prefb.size() < k)
            cout << -1 << endl;
        else
            cout << (prefa[k - 1] + prefb[k - 1]) << endl;
        return;
    }

    ll ans = INF;
    if (prefa.size() >= k && prefb.size() >= k)
        ans = prefa[k - 1] + prefb[k - 1];

    for (ll i = 0; i < min(k, (ll)prefboth.size()); i++)
    {
        ll curr = prefboth[i];
        ll left = k - i - 1;

        if (left > 0 && prefa.size() >= left && prefb.size() >= left)
        {
            curr += prefa[left - 1] + prefb[left - 1];
            ans = min(ans, curr);
        }
//        else if(prefa.empty)
        if (i >= k - 1)
