...
ll util(ll l, ll r, ll val)
{
    ...
    ll n = l + (r - l) / 2;
    for (ll i = l; i <= n; i++) cnt[str[i] - 'a'][0]++;
    for (ll i = n + 1; i <= r; i++) cnt[str[i] - 'a'][1]++;
//    debug2(cnt[0][0], cnt[0][1]);
    ll sz = r - l + 1;
    ll ans = util(l, n, val + 1) + (sz / 2) - cnt[val][1];
    ans = min(ans, util(n + 1, r, val + 1) + (sz / 2) - cnt[val][0]);
    return ans;
}
void solve()
{
