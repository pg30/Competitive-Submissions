...
{
    citizen[node] = arr[node];
    for (auto i : graph[node])
    {
        if (i != par)
        {
            dfs(i, node);
            citizen[node] += citizen[i];
        }
    }
    ll good = citizen[node] + brr[node];
    good /= 2;
    ll bad = citizen[node] - good;
    temp[node] = {good, bad};
}
bool dfs2(ll node, ll par)
{
    ...
    ll good = temp[node].fi, bad = temp[node].se;
    if (good < 0 || bad < 0 || good > citizen[node] || bad > citizen[node] || (good - bad) != brr[node] || (good + bad) != citizen[node])
        ans = 0;
    ll g = 0, b = 0;
    for (auto i : graph[node])
    {
    ...
    if (g + b != citizen[node] - arr[node] || g > good)
    {
        ans = 0;
    }
    return ans;
