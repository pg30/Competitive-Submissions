...
//---------------TEMPLATE ABOVE--------------//
ll getDiff(bitset<3> m1, bitset<3> m2)
{
    m1 = m1 ^ m2;
    return m1.count();
}
bool get(ll mask, ll bit)
{
    return ((mask & (1ll << bit)) != 0);
}
ll util(vector<bitset<3> > arr, ll m)
{
    ll n = arr.size();
    ll dp[n][9];
    for (ll i = 0; i < 9; i++)
    {
        dp[0][i] = getDiff(arr[0], bitset<3>(i));
        // debug3(arr[0], bitset<3>(i), dp[0][i]);
    }
    for (ll i = 1; i < n; i++)
    {
        for (ll j = 0; j < 9; j++)
        {
            ...
            for (ll k = 0; k < 9; k++)
            {
                ...
                for (ll a = 0; a < m - 1; a++)
                {
                    ll cnt = get(j, a) + get(j, a + 1) + get(k, a) + get(k, a + 1);
                    if (cnt % 2 == 0)
                    {
                        ok = 0;
                        break;
                ...
                {
//                    debug3(i, j, k);
                    dp[i][j] = min(dp[i][j], dp[i - 1][k] + getDiff(bitset<3>(j), arr[i]));
                }
            }
        }
    }
    ...
    if (n <= m)
    {
        ...
        for (ll i = 0; i < m; i++)
        {
            ...
            for (ll j = 0; j < n; j++)
                if (arr[j][i])
                    mask = mask | (1ll << (n - j - 1));
            curr.pb(bitset<3>(mask));
        }
        cout << util(curr, n) << endl;
        return;
    }
    else
    {
