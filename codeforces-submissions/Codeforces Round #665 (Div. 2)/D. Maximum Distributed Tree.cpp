...
{
    cin >> n;
    for (ll i = 0; i < n; i++) graph[i].clear(), dp[i] = 0;
    ans.clear();
    ...
    assert(ans.size() == n - 1);
    while (arr.size() < ans.size()) arr.pb(1);

    ll sz = arr.size() - ans.size();
    ll j = 0;
    for (; j <= sz; j++)
    {
        if (j)
            arr[j] = multiply(arr[j], arr[j - 1], MOD);
    }
    ...
    {
        fin = add(fin, multiply(arr[j++] , ans[i], MOD), MOD);
    }
    cout << fin << endl;
}
int main()
