...
void solve()
{
    ...
    stack<ll> small,big;
    vi ps(n,0),pb(n,0),ns(n,0),nb(n,0);
    for(ll i=n-1;i>=0;i--)
    {
        if(small.empty() || arr[small.top()]<arr[i])
            small.push(i);
        else
        {
        ...
        if(big.empty() || arr[big.top()]>arr[i])
            big.push(i);
        else
        {
            while(!big.empty() && arr[big.top()]<=arr[i])
            {
                ll ind = big.top();
                big.pop();
                pb[ind] = i;
...
//    printVector(nb);
    ...
    for(ll i=0;i<n;i++)
    {
        dp[i] = min(dp[i],dp[ps[i]]+1,dp[pb[i]]+1);
        dp[nb[i]] = min(dp[nb[i]],dp[i]+1);
        dp[ns[i]] = min(dp[ns[i]],dp[i]+1);
    }
    cout << dp[n-1] << endl;
}
int main()
