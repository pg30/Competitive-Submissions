...
//        debug2(d,u);
        ...
        for(auto &p : graph[u])
        {
            ...
            if(d+1<dp[v][type])
            {
                auto it = st.find({max(dp[v][0],dp[v][1]),v});
                if(it!=st.end())
                    st.erase(it);
                dp[v][type] = d+1;
                st.insert({max(dp[v][0],dp[v][1]),v});
//                debug2(max(dp[v][0],dp[v][1]),v);
            }
        }
    }
    cout << (max(dp[0][0],dp[0][1])>=INF?-1:max(dp[0][0],dp[0][1])) << endl;
    for(ll i=0;i<n;i++)
