...
struct segment_tree
{
    ...
    vector<ar<ll,4>> tree;
    vi arr;
    segment_tree(ll _n,vi a)
    {
        tree.assign(_n*4+4,{0,0,0,0});
        arr = a;
        n = arr.size();
    }

    void combine(ll ind,ll s,ll e)
    {
        if(s==e)
        {
            tree[ind][0] = arr[s];
        ...
        {
            tree[ind][0] = max(tree[2*ind+1][0],tree[2*ind+2][0],tree[2*ind+1][2]+tree[2*ind+2][0],tree[2*ind+1][0]-tree[2*ind+2][3]);

            tree[ind][1] = min(tree[2*ind+1][1],tree[2*ind+2][1],tree[2*ind+1][3]+tree[2*ind+2][1],tree[2*ind+1][1]-tree[2*ind+2][2]);

            tree[ind][2] = max(tree[2*ind+1][2],tree[2*ind+2][2],tree[2*ind+1][2]+tree[2*ind+2][2],tree[2*ind+1][0]-tree[2*ind+2][1]);

            tree[ind][3] = min(tree[2*ind+1][3],tree[2*ind+2][3],tree[2*ind+1][3]+tree[2*ind+2][3],tree[2*ind+1][1]-tree[2*ind+2][0]);
        }
        return;
    }
    ...
    void update(ll ind,ll s,ll e,ll pos,ll val)
    {
        ...
        ll m = s+(e-s)/2;
        update(2*ind+1,s,m,pos,val);
        update(2*ind+2,m+1,e,pos,val);
        combine(ind,s,e);
    }
...
//    for(ll i=0;i<5;i++)
//        debug4(st.tree[i][0],st.tree[i][1],st.tree[i][2],st.tree[i][3]);
//    cout << endl;
    cout << max(st.tree[0][0],st.tree[0][2]) << endl;

    vector<ar<ll,2>> query(q);
    for(ll i=0;i<q;i++)
    {
        cin >> query[i][0] >> query[i][1];

        ll a = query[i][0],b = query[i][1];
        a--;b--;
