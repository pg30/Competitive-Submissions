...
vi util(ll n,ll k)
{
    ...
    for(ll i=0;i<=n;i++)
    {
        ...
        if(i<k)
            ...
            ans[i] = divide(multiply(ans[i-1],i,MODT),i-k,MODT);
//            ans[i] = (ans[i-1]*i)/(i-2);
    }
//    printVector(ans);
    return ans;
...
void solve()
{
    ...
    for(ll i=0;i<n;i++)
    {
        arr[i][0] = lower_bound(all(temp),arr[i][0])-temp.begin();
        arr[i][1] = lower_bound(all(temp),arr[i][1])-temp.begin();
    ...
    {
        b.add(arr[i][1],-1);
        ll cnt = b.query(temp.size())-b.query(arr[i][0]-1);
        assert(cnt<n+3);
        ans = add(ans,cr[cnt],MODT);
//        cout << cnt << " " << cr[cnt] << endl;
    }
