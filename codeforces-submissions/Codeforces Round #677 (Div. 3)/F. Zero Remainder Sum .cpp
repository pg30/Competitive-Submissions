...
void solve()
{
    ...
    ll dp[n+5][m+5][k+5][m/2+5];
    fo(i,n+5) fo(j,m+5) fo(rem,k+5) fo(cnt,m/2+5) dp[i][j][rem][cnt] = -INF;

    dp[0][0][0][0] = 0;
    fo(i,n) fo(j,m) fo(rem,k) fo(cnt,m/2+1)
    {
        if(dp[i][j][rem][cnt]==-INF) continue;
        ll ni = (j==m-1?i+1:i);
        ll nj = (j==m-1?0:j+1);

        if(ni==i)
        {
            dp[ni][nj][rem][cnt] = max(dp[ni][nj][rem][cnt],dp[i][j][rem][cnt]);
            if(cnt+1<=m/2)
            {
                dp[ni][nj][(rem+arr[i][j])%k][cnt+1] = max(dp[ni][nj][(rem+arr[i][j])%k][cnt+1],dp[i][j][rem][cnt]+arr[i][j]);
            }
        }
        else
        {
            dp[ni][nj][rem][0] = max(dp[ni][nj][rem][0],dp[i][j][rem][cnt]);
            if(cnt+1<=m/2)
                dp[ni][nj][(rem+arr[i][j])%k][0] = max(dp[ni][nj][(rem+arr[i][j])%k][0],dp[i][j][rem][cnt]+arr[i][j]);
        }
    }
