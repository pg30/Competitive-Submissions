...
void solve()
{
    ...
    fo(i,n)
    {
        ...
        while(!st.empty())
        {
            ar<ll,2> curr = *st.begin();
            dis[i][curr[1]] = curr[0];
            st.erase(st.begin());

            for(auto j : graph[curr[1]])
            {
                if(dis[i][j[0]]>curr[0]+j[1])
                {
                    auto it = st.find({dis[i][j[0]],j[0]});
                    if(it!=st.end())
                        st.erase(it);

                    dis[i][j[0]] = curr[0]+j[1];
                    st.insert({dis[i][j[0]],j[0]});
                }
            }
        }
    }
    ...
    for(auto i : edge)
    {
        ...
        for(auto j : q)
        {
            ll a = j[0],b = j[1];
            ll now = dis[a][b];
            now = min(now,dis[a][u]+dis[b][v],dis[a][v]+dis[b][u]);
            curr += now;
        }
        ans = min(ans,curr);
    }
