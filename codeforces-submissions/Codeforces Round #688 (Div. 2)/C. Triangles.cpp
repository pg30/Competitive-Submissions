...
void solve()
{
    ...
    cin >> n;
    ll arr[n][n];
    ar<ll,2> row[n][10],col[n][10];
    vi posr[10],posc[10];
    for(ll i=0;i<n;i++)
        for(ll j=0;j<10;j++)
        {
            row[i][j][0] = col[i][j][0] = INF;
            row[i][j][1] = col[i][j][1] = -INF;
        }
    ...
    for(ll i=0;i<n;i++)
    {
        for(ll j=0;j<n;j++)
        {
            ...
            ll dig = arr[i][j];
            row[i][dig][0] = min(row[i][dig][0],j);
            row[i][dig][1] = max(row[i][dig][1],j);

            col[j][dig][0] = min(col[j][dig][0],i);
            col[j][dig][1] = max(col[j][dig][1],i);
    ...
    for(ll i=0;i<n;i++)
    {
        for(ll j=0;j<10;j++)
        {
            ll minr = row[i][j][0],maxr = row[i][j][1];
            if(minr==INF) continue;
            //bool one = posr[j][0]==posr[j][posr[j].size()-1];
            ll base = max(maxr-0,n-1-minr);
            ll height = 0;
            if(posr[j].size()>=1)
                height = max(i-posr[j][0],posr[j][posr[j].size()-1]-i);
            ans[j] = max(ans[j],base*height);
    ...
    for(ll i=0;i<n;i++)
    {
        for(ll j=0;j<10;j++)
        {
            ll minr = col[i][j][0],maxr = col[i][j][1];
            if(minr==INF) continue;
            //bool one = posr[j][0]==posr[j][posr[j].size()-1];
            ll base = max(maxr-0,n-1-minr);
            ll height = 0;
            if(posc[j].size()>=1)
                height = max(i-posc[j][0],posc[j][posc[j].size()-1]-i);
            ans[j] = max(ans[j],base*height);
