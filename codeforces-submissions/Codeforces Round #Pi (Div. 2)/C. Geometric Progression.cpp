...
void solve()
{
    ...
    ll dp[n][3] = {0};
    dp[0][0] = 1;
    dp[1][0] = 1 + (arr[0] == arr[1]);
    if (arr[1] % k == 0 && arr[1] / k == arr[0])
        dp[1][1] = 1;
    ll ans = 0;
    for (ll i = 2; i < n; i++)
    {
        dp[i][0] += 1;
        if (arr[i] % k == 0 && pos.find(arr[i] / k) != pos.end())
        {
            dp[i][1] = dp[pos[arr[i] / k]][0];
            dp[i][2] = dp[pos[arr[i] / k]][1];
        }
        if (pos.find(arr[i]) != pos.end())
        {
            dp[i][0] += dp[pos[arr[i]]][0];
            dp[i][1] += dp[pos[arr[i]]][1];
            dp[i][2] += dp[pos[arr[i]]][2];
            pos[arr[i]] = i;
        }
        else
            pos[arr[i]] = i;
