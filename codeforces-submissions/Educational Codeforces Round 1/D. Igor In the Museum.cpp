...
//---------------TEMPLATE ABOVE--------------//
const int maxn = 1004, maxm = 1004;
ll dp[maxn][maxm];
pi parent[maxn][maxm];
ll size[maxn][maxm];
void make_set(pi a, ll val)
{
    parent[a.fi][a.se] = a;
    size[a.fi][a.se] = 1;
    dp[a.fi][a.se] = val;
...
pi find(pi a)
{
    if (parent[a.fi][a.se] == a)
        return a;
    return parent[a.fi][a.se] = find(parent[a.fi][a.se]);
}
void unionf(pi a, pi b)
{
    a = find(a);
    b = find(b);
    ...
    {
        parent[a.fi][a.se] = b;
        size[b.fi][b.se] += size[a.fi][a.se];
        dp[b.fi][b.se] += dp[a.fi][a.se];
//        dp[a.fi][a.se] = 0;
    }
    else
    {
...
//                debug3(i, j, cnt);
    ...
    for (ll i = 0; i < n; i++)
    {
        for (ll j = 0; j < m; j++)
        {
            if (arr[i][j] == '.')
            {
                if (j > 0 && arr[i][j - 1] == '.') unionf(mp(i, j), mp(i, j - 1));
                if (j < m - 1 && arr[i][j + 1] == '.') unionf(mp(i, j), mp(i, j + 1));
                if (i > 0 && arr[i - 1][j] == '.') unionf(mp(i, j), mp(i - 1, j));
                if (i < n - 1 && arr[i + 1][j] == '.') unionf(mp(i, j), mp(i + 1, j));
            }
        }
    }
