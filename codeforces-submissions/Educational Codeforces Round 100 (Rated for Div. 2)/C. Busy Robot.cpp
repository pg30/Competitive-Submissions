...
void solve()
{
    ...
    ll ti = 0,pos = 0;
    ll ans = 0,prev=-1,prev_pos=0;
    for(ll i=0;i<n;i++)
    {
        if(arr[i][0]>=ti)
        {
            ti = arr[i][0] + abs(arr[i][1]-pos);
            prev_pos = pos;
            pos = arr[i][1];
            ll next = (i+1<n)?arr[i+1][0]:INF;
            if(next>=ti)
            {
                ans++;
                //debug1(i);
        ...
        {
            ll reach;
            if(arr[i][1]<min(pos,prev_pos) || arr[i][1]>max(pos,prev_pos))
                continue;
            reach = ti - abs(pos-arr[i][1]);
            ll next = (i+1<n)?arr[i+1][0]:INF;
            if(reach>=arr[i][0] && reach<=next)
            {
                ans++;
                //debug1(i);
            }
