...
//---------------TEMPLATE ABOVE--------------//
ll dp[19][19][2];
ll util(ll pos,vi &nums,ll cnt,bool f1,string num)
{
    if(pos==nums.size())
    {
        bool ok = cnt <= 3; 
    ...
    for(ll i=0;i<=dig;i++)
    {
        ans += util(pos+1,nums,cnt+(i!=0),f1 && (i==nums[pos]),num+char(i+'0'));
    }
    return dp[pos][cnt][f1] = ans;
}
void reset()
{
    for(ll i=0;i<19;i++) for(ll j=0;j<19;j++) dp[i][j][0] = dp[i][j][1] = -1;
}
void solve()
{
    ll l,r;
