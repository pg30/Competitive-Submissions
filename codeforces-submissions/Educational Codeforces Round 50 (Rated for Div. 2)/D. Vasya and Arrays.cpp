...
void solve()
{
    ...
    ll apref[n] = {0}, bpref[m] = {0};
    apref[0] = arr[0];
    bpref[0] = brr[0];
    for (ll i = 1; i < n; i++) apref[i] += apref[i - 1] + arr[i];
    for (ll i = 1; i < m; i++) bpref[i] += bpref[i - 1] + brr[i];
    ...
    for (ll i = 0; i < m; i++)
    {
        if (set.find(bpref[i]) != set.end())
        {
            ans++;
            set.erase(set.find(bpref[i]));
        }
    }
    if (ans == 0 || apref[n - 1] != bpref[m - 1]) ans = -1;
    cout << ans << endl;
}
int main()
{
