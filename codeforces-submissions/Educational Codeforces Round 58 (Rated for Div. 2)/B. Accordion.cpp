...
int main()
{
    ...
    while(t--)
    {
        string str;
        cin >> str;
        ll n = str.length(),first_open=-1,last_close=-1;
        std::vector<ll> a,b,c;
        for(ll i=0;i<n;i++)
        {
            if(first_open==-1 && str[i]=='[')
                first_open = i;
            if(str[i]==']')
                last_close = i;
            if(str[i]==':')
        ...
        if(first_open>last_close || first_open==-1 || last_close==-1)
        {
            cout << -1 << endl;
            continue;
        }
        ll ans = 0;
        ans+=2;
        str = str.substr(first_open,last_close-first_open+1);
        ll first_c=-1,last_c=-1;
        ...
        ans+=2;
        str = str.substr(first_c,last_c-first_c+1);
        for(ll i=0;i<str.length();i++)
        {
            if(str[i]=='|')
            {
