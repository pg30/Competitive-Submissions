...
int main()
{
    ...
    while(t--)
    {
        ll n,x,maxi = -1,maxd,temp;
        cin >> n >> x;
        std::vector<ll> blow(n),grow(n);
        for(ll i=0;i<n;i++){ cin >> blow[i] >> grow[i]; maxi = max(maxi,blow[i]);}
        maxd = grow[0]-blow[0];
        for(ll i=1;i<n;i++)
        {
                maxd = min(maxd,grow[i]-blow[i]);
        }
        if(x<=maxi)
        {
            cout << 1 << endl;
