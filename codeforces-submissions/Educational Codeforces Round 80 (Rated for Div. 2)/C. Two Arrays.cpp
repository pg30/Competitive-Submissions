...
ll solve(ll pos,ll ma)
{
    ...
    if(ma<=0)
        return 0;
    if(dp[pos][ma]!=-1) return dp[pos][ma];
    ll ans = add(solve(pos-1,ma),solve(pos,ma-1),MOD);
    return dp[pos][ma]=singlemod(ans,MOD);
}
int main()
{
    fastio();
...
//    ll cases = t;
    while(t--)
    {
        ...
        cout << singlemod(solve(2*m-1,n),MOD) << endl;
