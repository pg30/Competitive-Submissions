...
int main()
{
    ...
    while (t--)
    {
        ll n;
        cin >> n;
        std::vector<pair<ull, ull> > arr(n);
        for (ll i = 0; i < n; i++)
            cin >> arr[i].fi >> arr[i].se;

        ull sum[n] = {0};
        if (arr[0].fi >= arr[n - 1].se)
            sum[0] = arr[0].fi - arr[n - 1].se;
        else
            sum[0] = 0;
        //sum[0] = max(0ll, arr[0].fi - arr[n - 1].se);
        for (ll i = 1; i < n; i++)
        {
            sum[i] = sum[i - 1];
            if (arr[i].fi >= arr[i - 1].se)
                sum[i] += (arr[i].fi - arr[i - 1].se);
//            sum[i] = sum[i - 1] + max(0ll, arr[i].fi - arr[i - 1].se);
        }
        // for (ll i = 0; i < n; i++)
        //     cout << sum[i] << " ";
        // cout << endl;
