...
void solve()
{
    ...
    pair<ll, double> ans = {1, abs(t - h)};
    ll temp = (h + c) / 2;
    if (abs(t - temp) < ans.se)
    {
        ans = {2, abs(t - temp)};
    }
    ...
    a = abs(a);
//    debug1(a);
    ll p = max(0.0, 2 * floor(a) - 1), q = max(0.0, 2 * ceil(a) - 1);
//    debug2(p, q);
    long double ans1 = (((p + 1) / 2) * h + (p / 2) * c) * 1.0 / p;
    long double ans2 = (((q + 1) / 2) * h + (q / 2) * c) * 1.0 / q;
    ans1 = abs(ans1 - t);
    ans2 = abs(ans2 - t);
    pair<ll, double> fin;
    if (ans1 <= ans2)
