...
void solve()
{
    ...
    for (ll i = 0; i < n; i++)
    {
        for (ll j = 0; j < 61; j++)
        {
            ...
            {
                dp[i][j][0] = max(0ll, dp[i - 1][j][0] + arr[i - 1]);
            }
        }
    }
    ...
    for (ll i = n - 1; i >= 0; i--)
    {
        for (ll j = 0; j < 61; j++)
        {
            ...
            {
                dp[i][j][1] = max(0ll, dp[i + 1][j][1] + arr[i + 1]);
            }
        }
    }
    ...
    // for (ll i = 0; i < n; i++)
    // {
    //     debug3(i, arr[i], dp[i][arr[i] + 30][0]);
    // }
    ...
    // for (ll i = 0; i < n; i++)
    // {
    //     debug3(i, arr[i], dp[i][arr[i] + 30][1]);
    // }

    ll ans = 0;
    for (ll i = 0; i < n; i++)
    {
//        debug2(i, dp[i][arr[i] + 30][0] + dp[i][arr[i] + 30][1]);
        // if (i == 0)
        // {
        //     ans = max(ans, dp[i][arr[i] + 30][1]);
        // }
        // else if (i == n - 1)
        // {
        //     ans = max(ans, dp[i][arr[i] + 30][0]);
        // }
        ans = max(ans, dp[i][arr[i] + 30][0] + dp[i][arr[i] + 30][1]);
    }
    cout << ans << endl;
}
int main()
