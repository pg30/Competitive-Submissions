...
const int maxn = 35, maxy = 35;
ll dp[maxn][maxy];
void dfs(ll x, ll y, ll dis, ll n, ll m)
{
    dp[x][y] = dis;
    for (ll i = 0; i < 2; i++)
    {
...
void solve()
{
    ...
    for (ll i = 1; i <= n; i++)
    {
        for (ll j = 1; j <= m; j++)
        {
            dp[i][j] = max(0ll, min(dp[i][j], maxi - dp[i][j] + 1ll));
//            debug3(i, j, dp[i][j]);
        }
    }
    ...
    // for (ll i = 0; i <= maxi; i++)
    //     debug3(i, cnt[i][0], cnt[i][1]);
    ll ans = 0;
    for (ll i = 0; i <= maxi; i++)
        ans += min(cnt[i][0], cnt[i][1]);
    if ((n + m - 1) % 2 == 1)
    {
        ans -= min(cnt[(n + m) / 2][0], cnt[(n + m) / 2][1]);
    }
    cout << ans << endl;
}
int main()
