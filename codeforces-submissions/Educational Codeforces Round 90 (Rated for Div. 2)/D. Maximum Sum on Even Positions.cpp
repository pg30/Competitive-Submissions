...
void solve()
{
    ...
    dp[1][0] = dp[1][1] = dp[1][2] = arr[0];

    for (ll i = 2; i <= n; i++)
    {
        dp[i][0] = dp[i - 1][0] + (i % 2 == 1 ? arr[i - 1] : 0);
        dp[i][1] = max(dp[i - 2][0], dp[i - 2][1]) + (i % 2 == 1 ? arr[i - 2] : arr[i - 1]);
        dp[i][2] = max(dp[i - 1][0], dp[i - 1][1], dp[i - 1][2]) + (i % 2 == 1 ? arr[i - 1] : 0);
    }
