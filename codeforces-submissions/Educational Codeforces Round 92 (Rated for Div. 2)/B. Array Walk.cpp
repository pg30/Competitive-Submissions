...
ll dp[maxn][6][2];
ll util(ll pos, ll k, ll z, ll z1, bool left)
{
    ll move = pos + (left ? 2 * (z1 - 1) + 2 : 2 * z1);
//    debug5(pos, z1, k, move, left);
    if (move == k + 1) return arr[pos];
    if (dp[pos][z1][left] != -1) return dp[pos][z1][left];
    ll ans = 0;
    if (z1 < z && pos > 0 && !left)
        ans = arr[pos] + util(pos - 1, k, z, z1 + 1, 1);
    ans = max(ans, arr[pos] + util(pos + 1, k, z, z1, 0));
    return dp[pos][z1][left] = ans;
}
void solve()
{
    cin >> n >> k >> z;
    ...
    cout << arr[0] + util(1, k - 1, z, 0, 0) << endl;
}
int main()
{
    fastio();
