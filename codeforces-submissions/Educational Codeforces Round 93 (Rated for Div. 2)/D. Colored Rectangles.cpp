    ...
    for (ll i = 0; i <= a + 1; i++)
    {
        for (ll j = 0; j <= b + 1; j++)
        {
            for (ll k = 0; k <= c + 1; k++)
            {
                dp[i][j][k] = 0;
                if (i <= a && j <= b && i > 0 && j > 0)
                    dp[i][j][k] = max(dp[i][j][k], dp[i - 1][j - 1][k] + arr[0][i - 1] * arr[1][j - 1]);
                if (i <= a && k <= c && i > 0 && k > 0)
                    dp[i][j][k] = max(dp[i][j][k], dp[i - 1][j][k - 1] + arr[0][i - 1] * arr[2][k - 1]);
                if (j <= b && k <= c && j > 0 && k > 0)
                    dp[i][j][k] = max(dp[i][j][k], dp[i][j - 1][k - 1] + arr[1][j - 1] * arr[2][k - 1]);
                ans = max(ans, dp[i][j][k]);
//                debug4(i, j, k, dp[i][j][k]);
            }
        }
    }
    cout << ans << endl;
