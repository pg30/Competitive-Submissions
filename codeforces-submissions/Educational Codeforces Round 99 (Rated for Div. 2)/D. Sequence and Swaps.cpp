...
ll util(ll pos,ll maxi,ll x,ll n)
{
    if(pos==n)
    {
        return 0;
    ...
    if(dp[pos][maxi][x]!=-1)
        return dp[pos][maxi][x];
    //debug3(pos,maxi,x);
    ll ans = INF;
    if(arr[pos]>=maxi)
    {
        ans = min(ans,util(pos+1,arr[pos],x,n));
        if(x>=maxi && x<arr[pos])
            ans = min(ans,1+util(pos+1,x,arr[pos],n));
    }
    else
        return dp[pos][maxi][x] = INF;
    return dp[pos][maxi][x] = ans;
}
void solve()
{
    ll n,x;
    ...
    arr.resize(n);
    cin >> arr;
    ll ans = util(1,arr[0],x,n);
    if(x<arr[0])
        ans = min(ans,1+util(1,x,arr[0],n));
    if(ans>=INF)
        cout << -1 << endl;
    else 
        cout << ans << endl;
