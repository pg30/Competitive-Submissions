...
int main()
{
    ...
    { 
        ...
        do
        {
           // printArray(temp,4);
            string aisehi=str;
            for(ll i=0;i<aisehi.length();i++)
            {
                aisehi[i] = temp[aisehi[i]-'0'];
            }
//            cout << aisehi << endl;
            ll i=s.fi,j=s.se,k=0;
            ll ok = 0;
            while(i>=0 && i<n && j>=0 && j<m && k<aisehi.length())
            {
                if(e.fi==i && e.se==j)
                {
                    //debug5(i,j,k,e.fi,e.se);  
                    ok = 1;
                    break;
                }
                if(arr[i][j]=='#')
        ...
        while(next_permutation(alla(temp,4)));
        cout << ans << endl;
