...
void solve()
{
    ...
    for(auto i : arr)
    {
        ...
        for(ll j=0;j<60;j++)
        {
            if((i & (1ll << j)))
            {
                a = add(a,multiply((1ll << j),cnt[j][1],MOD),MOD);
                b = add(b,multiply(1ll << j,cnt[j][1]+cnt[j][0],MOD),MOD);
            }
            else
                b = add(b,multiply(1ll << j,cnt[j][1],MOD),MOD);
        }
        ans = add(ans,multiply(a,b,MOD),MOD);
    }
    cout << ans << endl;
}
int main()
