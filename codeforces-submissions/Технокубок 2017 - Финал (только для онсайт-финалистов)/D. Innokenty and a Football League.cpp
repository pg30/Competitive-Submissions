...
void solve()
{
    ...
    string arr[n][2];
    map<string,vector<pair<string,ll>>> mp;
    for(ll i=0;i<n;i++)
    {
        cin >> arr[i][0] >> arr[i][1];
        arr[i][0] = arr[i][0].substr(0,3);
        arr[i][1] = arr[i][0].substr(0,2)+arr[i][1].substr(0,1);
        mp[arr[i][0]].pb({arr[i][1],i});
    }
    set<string> used;
    vector<string> ans(n);
    for(auto &i : mp)
    ...
    while(1)
    {
        ...
        for(auto &i : mp)
        {
            if(i.se.empty()) continue;
            if(used.count(i.fi) && used.count(i.se[0].fi))
            {
                cout << "NO" << endl;
                return;
            }
            if(used.count(i.fi))
            {
                ans[i.se[0].se] = i.se[0].fi;
                used.insert(i.se[0].fi);
                i.se.clear();
                ok = 1;
            }
        }
